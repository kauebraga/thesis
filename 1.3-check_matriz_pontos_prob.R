...
probs <- c("8980104e417ffff", "8980104e4abffff")
probs <- c("8980104e0d3ffff", "8980104e46bffff")

# find snap for these guys
points_prob <- points %>% filter(id %in% probs)
...
int_dt1 <- detailed_itineraries(r5r_core = r5r_core,
                                origins = points %>% filter(id %in% probs[1]),
...
int_dt2 <- detailed_itineraries(r5r_core = r5r_core,
                                origins = points %>% filter(id %in% probs[2]),
                                destinations = point_downtown,
                                mode = mode,
                                departure_datetime = departure_datetime,
                                max_walk_dist = max_walk_dist,
